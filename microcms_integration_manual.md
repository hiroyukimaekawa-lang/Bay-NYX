# microCMS動的化 導入マニュアル（旧版・参考資料）

> [!IMPORTANT]
> **このマニュアルは現在の構成とは異なります。運用手順は [MICROCMS_SETUP.md](MICROCMS_SETUP.md) を参照してください。**
>
> このドキュメントはフロントエンドのJavaScriptにAPIキーを直接書く方法を説明していますが、
> その方法では **APIキーがブラウザから誰でも見える状態** になるため、現在は採用していません。
> 現在はAPIキーをサーバー側（`functions/api/baynyx.js`）で保持し、
> フロントエンドは `/api/baynyx` を呼ぶだけの構成になっています。
>
> microCMSの基本的な考え方の参考資料として残しています。
